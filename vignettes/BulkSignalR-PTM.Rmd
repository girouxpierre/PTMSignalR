...
bsrdmResp <- prepareDataset(counts = dfProt, PTM = dfPhos_complete, 
                    single = T, symbolPh.col = 1, pos.col = 2, normalize = T, method="TC",min.count=0,min.LR.found=10)
bsrdm.compResp <- as.BSRDataModelCompPTM(bsrdmResp)

# indices resp vs non resp
indPhosResp <- which(colnames(bsrdm.compResp@PTM) %in% unlist(phosResp))
indPhosNoResp <- which(colnames(bsrdm.compResp@PTM) %in% unlist(phosNoResp))
...
bsrdm.compResp <- addClusterCompPTM(bsrdm.compResp,bsrccResp,"resp.versus.all")
bsrdm.compResp
