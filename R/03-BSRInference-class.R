...
setClass("BSRInference",
         ...
         prototype=list(
             LRinter=data.frame(L="A", R="B", pw.id="123", pw.name="one pw",
                                ...
                                len=50, rank.corr=0.6, target.genes.phos=list(c("C","D")),
                                stringsAsFactors=FALSE),
             ligands=list("A"),
             receptors=list("B"),
             t.genes=list(c("a","b","c")),
             tg.corr=list(c(-0.5,0.1,0.8)),
             phospho.genes=list(c("a","b","c")),
             pg.corr=list(c(-0.5,0.1,0.8)),
             inf.param=list()
         ))
...
setMethod("show", "BSRInference",
          function(object) {
              cat("\n Reference database: ", object@inf.param$reference, "\n", sep="")
...
if (!isGeneric("pGenes")) {
  if (is.function("pGenes"))
    fun <- pGenes
  else
    fun <- function(x) standardGeneric("pGenes")
  setGeneric("pGenes", fun)
}
#' Phospho genes accessor
#'
#' @name pGenes
#' @aliases pGenes,BSRInference-method
#' @param x BSRInferance object
#' @export
setMethod("pGenes", "BSRInference", function(x) x@phospho.genes)

if (!isGeneric("pGenes<-")) {
  if (is.function("pGenes<-"))
    fun <- `pGenes<-`
  else
    fun <- function(x,value) standardGeneric("pGenes<-")
  setGeneric("pGenes<-", fun)
}
...
#' @param value value to be set BSRInference
#' @keywords internal 
setMethod("pGenes<-", "BSRInference", function(x, value){
  x@phospho.genes <- value
  methods::validObject(x)
  x
})

if (!isGeneric("pgCorr")) {
  if (is.function("pgCorr"))
    fun <- pgCorr
  else
    fun <- function(x) standardGeneric("pgCorr")
  setGeneric("pgCorr", fun)
}
#' Target gene correlations accessor
#'
#' @name pgCorr
...
#' @param x BSRInference object
#' @export
setMethod("pgCorr", "BSRInference", function(x) x@pg.corr)

if (!isGeneric("pgCorr<-")) {
  if (is.function("pgCorr<-"))
    fun <- `pgCorr<-`
  else
    fun <- function(x,value) standardGeneric("pgCorr<-")
  setGeneric("pgCorr<-", fun)
}
...
#' @param value value to be set for bsrinf
#' @keywords internal
setMethod("pgCorr<-", "BSRInference", function(x, value){
  x@pg.corr <- value
  methods::validObject(x)
  x
})
