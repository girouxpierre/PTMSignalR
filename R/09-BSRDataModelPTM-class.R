...
setMethod("show", "BSRDataModelPTM",
          function(object) {
            cat("\n Expression values are log2-transformed: ", object@log.transformed,
                "\n", sep="")
            cat("\n Normalization method: ", object@normalization, "\n", sep="")
            cat("\n Organism : ", object@initial.organism,"\n", sep="")
...
if (!isGeneric("symPos")) {
  if (is.function("symPos"))
    ...
    fun <- function(x) standardGeneric("symPos")
  setGeneric("symPos", fun)
}
#' Normalized count matrix accessor
#'
#' @name symPos
...
if (!isGeneric("symPos<-")) {
  if (is.function("symPos<-"))
    ...
    fun <- function(x, value) standardGeneric("symPos<-")
  setGeneric("symPos<-", fun)
}
#' Normalized count matrix setter (internal use only)
#'
...
if (!isGeneric("single")) {
  if (is.function("single"))
    fun <- single
  else
    fun <- function(x) standardGeneric("single")
  setGeneric("single", fun)
}
#' log.transformed accessor
#'
#' @name single
...
setMethod("learnParameters", "BSRDataModelPTM", function(obj, plot.folder = NULL,
  ...
  obj@param$n.rand.RP <- as.integer(n.rand.RP)
  if (obj@param$n.rand.RP < 1)
    stop("Parameter n.rand.RP must be an integer > 0")
  ...
  obj@param$single <- obj@single #??
  obj@param$single <- single #??
  ...
  cat("\n learnParameters - BSRDMP - av .getEmpiricalNullCorrLR")
  ...
  if (is.null(trainModel)){
    ...
    if (verbose){
      ...
      if (is.null(np))
        cat("\n   Censored normal estimation did not converge\n")
      else
        cat("\n   Censored normal D=", np$D,
            ", Chi2=", np$Chi2, "\n", sep="")
      if (is.null(mp))
        cat("\n   Censored Mixture of normals estimation did not converge\n")
      else
    ...
    if ((npchi < 1.25*mpchi) && (npchi < 2*kpchi)){
      ...
      trainModel <- .getKernelEmpiricalParam
      if (verbose)
        cat("\n   ==> select Gaussian kernel-based empirical\n")
    }
  }
  ...
  if (obj@param$quick) {
    ...
    ds.RT.null <- .getEmpiricalNull(obj@ncounts, obj@PTM,
                                    ...
                                    min.cor = obj@param$min.corr.LR)
    cat("\n learnParameters - BSRDMP - ap getEmpiricalNull")
    #cat(ds.RT.null)
    t <- ds.RT.null[[1]]
    #cat(t)
  ...
  if (obj@param$quick) {
    # RP correlations are assumed to be equal to LR correlations
    if (verbose)
      cat(paste0("Quick learning, receptor-target correlation null ","
                   distribution assumed to be equal to ligand-receptor...\n"))
    obj@param$RP.0$n <- obj@param$LR.0$n
    obj@param$RP.0$model <- obj@param$LR.0$model
  }
  else {
    ...
    ds.RP.null <- .getEmpiricalNullPTM(obj@ncounts,obj@PTM,
                                    ...
                                    single=obj@single,
                                    symPos=obj@symPos)
    cat("\n ds.RP.null: \n") #ne recoit rien de getEmpNP
    cat(unlist(ds.RP.null))
    t <- ds.RP.null[[1]]
    if (length(ds.RP.null) > 1)
      for (i in 2:length(ds.RP.null)) t <- rbind(t, ds.RP.null[[i]])
    cat("\n t:")
    cat(unlist(t))
    above <- unlist(strsplit(t$target.corr, split = "\\|"))
    cat("\n length :")
    ...
    if (null.model == "stable")
      # sub-sample randomized R-T correlations to limit compute time
      r.corrs <- sample(r.corrs, obj@param$LR.0$n)
    obj@param$RP.0$n <- length(r.corrs)
    
    # fit null model
    if (!is.null(plot.folder))
      file.name <- paste0(plot.folder,  "/",filename,"_RP-null.pdf")
    else
      file.name <- NULL
    cat("\n learnParameters - BSRDMP - av trainModelP")
    gp <- trainModel(r.corrs, "RP correlation (null)",
                     verbose = verbose, file.name = file.name)
    obj@param$RP.0$model <- gp
  }
...
setMethod("initialInference", "BSRDataModelPTM", function(obj, rank.p=0.55,
  ...
  inter <- .pValuesLRP(pairs, param(obj), rank.p=rank.p, fdr.proc=fdr.proc)
